...
/* Includes */
#include "/home/gabeg/scripts/programs/atlas/c++/extra/include/atlaswm.h"
...
// /* Declares */
// static uint16_t pidof(char *prog);
// static uint32_t log2(uint32_t val);
// static int      barkey(unsigned int val);
...
//     unsigned int brt_up;
//     unsigned int brt_down;
//     unsigned int vol_down;
//     unsigned int vol_mute;
//     unsigned int vol_up;
//     unsigned int trk_play;
//     unsigned int trk_next;
//     unsigned int trk_prev;
// } yokey;


// static const yokey ass = { 
//     .floating = XK_m,
//     .tiling   = XK_t,
//     .left     = XK_Left,
//     .right    = XK_Right,
//     .brt_up   = KEY_BRIGHT_UP,
//     .brt_down = KEY_BRIGHT_DOWN,
//     .vol_down = KEY_VOL_DOWN,
//     .vol_mute = KEY_VOL_MUTE,
//     .vol_up   = KEY_VOL_UP,
//     .trk_play = KEY_TRACK_PLAY,
//     .trk_next = KEY_TRACK_NEXT,
//     .trk_prev = KEY_TRACK_PREV
// };
...
// void barsignal(const Arg *arg)
// {
//     int      key = barkey(arg->ui);
//     uint16_t pid = pidof("atlas");

//     if ( pid > 0 ) {
//         sem_unlink("/semi");
//         sem_t *sem = sem_open("/semi", O_CREAT, 0644, key);
//         sem_close(sem);
//         kill(pid, SIGUSR1);
//     }
// }
...
// /* ************************************************************************** */
// /* */
// int barkey(unsigned int val)
// {
//     int key = -1;
//     int max = 1 << (5-1);
//     // int max = 1 << ((sizeof(tags)/sizeof(tags[0]))-1);
//     unsigned int screen = statusBarScreen;

//     FILE *fp = fopen("/home/gabeg/.config/dwm/stuff.txt", "a+");
//     fprintf(fp, "AHHHH 0x%0x\n", val);
//     fprintf(fp, "tags: %s %s %s %s %s\n", tags[0], tags[1], tags[2], tags[3], tags[4]);
//     fprintf(fp, "screen: 0x%0x | max: 0x%0x\n", screen, max);

//     if ( val == ass.left ) {
//         screen = statusBarScreen >> 1;
//         if ( screen == 0 )
//             screen = max;
//         key = SCREEN + log2(screen) + 1; /* Increment key to make it work with Atlas */
//         fprintf(fp, "left: 0x%0x 0x%0x 0x%0x 0x%0x\n", key, SCREEN, screen, log2(screen));
//     }
//     else if ( val == ass.right ) {
//         screen = statusBarScreen << 1;
//         if ( screen > max )
//             screen = 0x1;
//         key = SCREEN + log2(screen) + 1; /* Increment key to make it work with Atlas */
//         fprintf(fp, "right: 0x%0x 0x%0x 0x%0x 0x%0x\n", key, SCREEN, screen, log2(screen));
//     }
//     else if ( val == ass.brt_up )
//         key = BRIGHT_UP;
//     else if ( val == ass.brt_down )
//         key = BRIGHT_DOWN;
//     else if ( val == ass.trk_play )
//         key = TRACK_PLAY;
//     else if ( val == ass.trk_next )
//         key = TRACK_NEXT;
//     else if ( val == ass.trk_prev )
//         key = TRACK_PREV;
//     else if ( val == ass.vol_up )
//         key = VOL_UP;
//     else if ( val == ass.vol_down )
//         key = VOL_DOWN;
//     else if ( val == ass.vol_mute )
//         key = VOL_MUTE;
//     else {
//         screen = val;
//         key    = SCREEN + log2(screen) + 1; /* Increment key to make it work with Atlas */
...
//     fprintf(fp, "key: 0x%0x\n", key);
//     fclose(fp);

//     statusBarScreen = screen;
//     return key;
// }
