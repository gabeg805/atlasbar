...
/* ************************************************************************** */
/* Return the application corresponding to the given application */
Gtk::Widget * AtlasAppUtil::get_widget(NameApp *app)
{
    ...
    if ( event.empty() )
        return static_cast<Gtk::Widget*>(app->app);
    else
        return static_cast<Gtk::EventBox*>(app->app)->get_child();
}
...
int AtlasAppUtil::set_image(NameApp *app, void **widget)
{
    ...
    for ( i = 0; i < len; ++i ) {
        img[i].set(strvec[i]);
        AtlasAppUtil::set_margin(img[i], margin);
        AtlasAppUtil::set_padding(img[i], padding);
        AtlasAppUtil::set_background(img[i], background);
        AtlasAppUtil::set_foreground(img[i], foreground);
    }
