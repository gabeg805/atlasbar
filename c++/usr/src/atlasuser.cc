...
{
    AtlasUserApp *builder = new AtlasUserApp(6);
    builder->new_uapp("battery",    {get_battery_info,    battery_event,    NULL},              0);
    builder->new_uapp("wifi",       {get_wifi_info,       wifi_event,       NULL},              0);
    builder->new_uapp("volume",     {get_volume_info,     volume_event,     volume_signal},     ATLAS_SIG_VOL);
    builder->new_uapp("brightness", {get_brightness_info, brightness_event, brightness_signal}, ATLAS_SIG_BRT);
    builder->new_uapp("date",       {get_date_info,       NULL,             NULL},              0);
    builder->new_uapp("workspace",  {get_workspace_info,  NULL,             workspace_signal},  ATLAS_SIG_MON);
...
int workspace_signal(uint32_t key)
{
    ...
    std::cout << "sigmon: 0x" << std::hex << ATLAS_SIG_MON << std::endl;
    std::cout << "Keyand: 0x" << std::hex << (key&ATLAS_SIG_MASK) << std::endl;
    std::cout << "Keysub: 0x" << std::hex << (key^ATLAS_SIG_MON) << std::endl;
    if ( (key & ATLAS_SIG_MASK) == ATLAS_SIG_MON )
        return ((key ^ ATLAS_SIG_MON));
    else
        return ATLAS_SIG_DONE;
}
