...
void set_tag(int loc) {
    ...
    while ( tag_box[i] != 0 ) {
        ...
        if ( strcmp(str, tags[j]) == 0 )
            gtk_widget_override_background_color(tag_box[i], GTK_STATE_FLAG_NORMAL, &color);
        else 
            gtk_widget_override_background_color(tag_box[i], GTK_STATE_FLAG_NORMAL, &colorq);
...
void display_tags(int loc) {
    ...
    while ( tags[i] != 0 ) {
        ...
        gtk_label_set_attributes(GTK_LABEL(tag), Atlas.attr);
        ...
        gtk_box_pack_start(GTK_BOX(Atlas.bar), box, 0, 0, 0);
